import { sqliteTable, AnySQLiteColumn, index, text, integer, uniqueIndex, foreignKey, blob } from "drizzle-orm/sqlite-core"
...
(table) => [
	index("idx_audio_assets_goal").on(table.goalId),
]);
...
(table) => [
	index("idx_segments_story").on(table.storyId, table.idx),
]);
...
(table) => [
	index("idx_questions_goal").on(table.goalId),
]);
...
(table) => [
	index("idx_claim_cards_note").on(table.noteId),
]);
...
export const audioSegments = sqliteTable("audio_segments", {
	...
	assetId: text("asset_id").notNull().references(() => audioAssets.id, { onDelete: "cascade" } ),
	idx: integer().notNull(),
	mimeType: text("mime_type").notNull(),
	bytes: blob().notNull(),
	sha256: text(),
...
(table) => [
	index("idx_audio_segments_asset").on(table.assetId, table.idx),
]);
